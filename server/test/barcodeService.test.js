...
test('lookup returns a household match without calling the public lookup', async () => {
  const repo = fakeRepository({ '111': { name: 'Milk', category: 'Cold Things', note: '2 L' } });
  let publicCalls = 0;
  ...
  assert.deepEqual(result, { found: true, name: 'Milk', category: 'Cold Things', note: '2 L', source: 'household' });
...
test('lookup falls back to the public lookup on a household miss', async () => {
  ...
  const lookupPublicProduct = async (barcode) => {
    ...
    return { name: 'Baked Beans', category: null, note: '415 g' };
  };
  const service = createBarcodeService(repo, lookupPublicProduct);

  const result = await service.lookup('h1', '222');

  assert.deepEqual(result, { found: true, name: 'Baked Beans', category: null, note: '415 g', source: 'public' });
});
...
test('lookup returns not-found when both household and public miss', async () => {
  ...
  assert.deepEqual(result, { found: false, name: null, category: null, note: null, source: null });
});
